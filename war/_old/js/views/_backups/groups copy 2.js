...
function loadGroups()
{
	webpaige.con('get', '/network', null, 'Loading groups..', null, renderGroups);
}
...
function addGroup(groupName)
{
  ...
  var json = '{"name": "' + groupName + '"}';
	webpaige.con('post', '/network/'+uuid, json, 'Adding new group..', 'Group added.', loadGroups);
}
...
function updateGroup(groupName, uuid)
{
  ...
  {
	  console.log("group update action: ", data);
    loadGroups();
	}
	webpaige.con('put', '/network/'+uuid, json, 'Updating group', 'Group updated.', callback);
}
...
function deleteGroup(uuid)
{
  var callback = function(data)
  {
	  console.log("group delete action: ", data);
	  loadGroups();
	}
	webpaige.con('delete', '/network/'+uuid, null, 'Deleting group..', 'Group deleted', callback);
}
...
{
  var json = '{"key":"' + value + '"}';
	webpaige.con('post', '/network/searchPaigeUser', json, 'Searching members..', null, renderSearchResults);
}
...
function loadMembers(groupName, uuid, i)
{
  var callback = function(data)
  {
	...
	webpaige.con('get', '/network/'+uuid+'/members', null, 'Loading members..', null, callback);
}
...
function addMembers()
{
  ...
  var values = $('#searchResultsPane input:checkbox:checked').map(function ()
  {
	  if (this.value != 'on')
	  {
		  var callback = function(data)
		  {
			  console.log("member add action: ", data);
			}
			webpaige.con('post', '/network/'+groupUuid+'/members/'+this.value, null, 'Adding member(s)..', 'Member(s) added', callback);
	  }
	}).get();
}
...
function removeMember(uuid, memberUuid)
{
  var callback = function(data)
  {
	  console.log("member delete action: ", data);
	}
	webpaige.con('delete', '/network/'+uuid+'/members/'+memberUuid, null, 'Removing member(s)..', 'Member(s) removed', callback);
}
